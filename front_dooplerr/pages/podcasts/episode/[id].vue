<template>
    <div class="container max-w-screen-sm mx-auto mb-28">
        ...
        <div class="flex flex-col justify-center items-center px-8">
            <div v-if="episode.Podcasts && episode.Podcasts.length > 0"
                class="img-podcast w-full max-w-[340px] aspect-square rounded-md">
                <img :src="episode.Podcasts[0].thumbnail_path" alt="Image de l'épisode"
        ...
        <div v-if="episode" class="w-full flex flex-col items-center px-8">
            ...
            <div class="w-full bg-gray-800 rounded-full h-1 relative" @click="onProgressBarClick">
                ...
                <div class="absolute progress-dot top-0 left-0 transform -translate-x-1/2 -translate-y-1/2 bg-white w-4 h-4 rounded-full"
                    :style="{ left: calcProgressDotPosition }"></div>
            </div>
        ...
        <div class="mt-15 px-8" v-if="episode.Comments">
            <h2 class="text-base font-semibold italic text-white mb-4">
                ...
                {{ episode.Comments.length > 1 ? episode.Comments.length + ' commentaires' :
                    episode.Comments.length + ' commentaire' }}
            </h2>
            <!-- Form add comment -->
